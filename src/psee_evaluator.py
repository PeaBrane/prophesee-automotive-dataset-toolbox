...
def evaluate_folders(dt_folder, gt_folder, camera):
    ...
    def filter_boxes_fn(x): return filter_boxes(
        x, int(1e5), min_box_diag, min_box_side)
...
def main():
    ...
    parser.add_argument('gt_folder', type=str,
                        help='GT folder containing .npy files')
    parser.add_argument('dt_folder', type=str,
                        help='RESULT folder containing .npy files')
    parser.add_argument('--camera', type=str, default='GEN4',
                        help='GEN1 (QVGA) or GEN4 (720p)')
